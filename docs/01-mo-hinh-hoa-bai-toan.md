...
```mermaid
flowchart LR
    subgraph Thực tế Đại học
        S[Lớp học phần]
        C[Xung đột GV / Sinh viên]
        T[Ca học khả dụng]
    end
    
    subgraph Không gian Đồ thị
        V((Đỉnh / Vertex))
        E{Cạnh / Edge}
        Col[Màu sắc / Color]
    end
...
## 2. Phân tích Ràng buộc (Constraints Analysis)

Để đảm bảo tính khả thi và có thể áp dụng trực tiếp vào thực tiễn, không gian nghiệm của bài toán đồ thị bị giới hạn chặt chẽ bởi hai lớp ràng buộc sau:
...
```mermaid
graph LR
    A[Giao diện CLI / Web UI]
    -->|Gọi Tên Dataset: 'complex'|
    B(DemoData API Helper)

    B
    -->|Xây dựng Mảng & Trả về vector<Subject>|
    C{Lõi Giải Thuật Đồ Thị}

    C
    -->|Output|
    D[ScheduleItem Report]
```
